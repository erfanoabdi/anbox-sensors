...
static int sensor_device_poll_event_locked(SensorDevice* dev)
{
    ...
    if (dev->mSensorFWDevice->GetAccelerometerEvent(&ts, &x, &y, &z) == 0) {
        if (ts != last_TimeStamp[ID_ACCELEROMETER]) {
            ...
            last_TimeStamp[ID_ACCELEROMETER] = ts;
        }
    }
    if (dev->mSensorFWDevice->GetGyroscopeEvent(&ts, &x, &y, &z) == 0) {
        if (ts != last_TimeStamp[ID_GYROSCOPE]) {
    ...
    if (dev->mSensorFWDevice->GetMagnetometerEvent(&ts, &x, &y, &z, &rx, &ry, &rz, &tmp) == 0) {
        if (ts != last_TimeStamp[ID_MAGNETIC_FIELD]) {
            ...
            last_TimeStamp[ID_MAGNETIC_FIELD] = ts;
        }
        if (ts != last_TimeStamp[ID_MAGNETIC_FIELD_UNCALIBRATED]) {
            ...
            last_TimeStamp[ID_MAGNETIC_FIELD_UNCALIBRATED] = ts;
        }
    }
    if (dev->mSensorFWDevice->GetOrientationEvent(&ts, &tmp) == 0) {
        if (ts != last_TimeStamp[ID_DEVICE_ORIENTATION]) {
